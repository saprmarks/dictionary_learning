...
class StandardTrainer(SAETrainer):
    ...
    def __init__(self,
                 ...
                 lr_decay_steps_fraction:Optional[float]=0.2,
                 final_lr_fraction:Optional[float]=0.1,
                 steps: Optional[int]=None, # total  of steps to train for
        ...
        """Options:
        warump_steps: LR linear warmup period at start of training and after each resample
        sparsity_warmup_steps: Sparsity linear warmup period at start of training
        lr_decay_steps_fraction: LR linear decay for the last fraction of training"""
        super().__init__(seed)
        ...
        if lr_decay_steps_fraction is not None:
            assert steps is not None, "total number of steps must be specified for lr decay"
            assert resample_steps is None, "lr decay not implemented for resampling"
            assert lr_decay_steps_fraction < 1 and lr_decay_steps_fraction > 0
            assert final_lr_fraction <= 1 and final_lr_fraction >= 0

        self.steps = steps
        self.sparsity_warmup_steps = sparsity_warmup_steps
        self.lr_decay_steps_fraction = lr_decay_steps_fraction
        ...
        if resample_steps is None:
            def warmup_fn(step):
                warmup_scale = min(step / warmup_steps, 1.)

                if self.lr_decay_steps_fraction is not None:
                    cooldown_start = self.steps * (1 - self.lr_decay_steps_fraction)
                    if step >= cooldown_start:
                        cooldown = 1.0 + (self.final_lr_fraction - 1.0) * (step - cooldown_start) / (self.steps - cooldown_start)
                        return max(cooldown, self.final_lr_fraction)
                return warmup_scale
        else:
            def warmup_fn(step):
    ...
    def loss(self, x, step: int, logging=False, **kwargs):
        ...
        if self.sparsity_warmup_steps is not None:
            sparsity_scale = min(step / self.sparsity_warmup_steps, 1.)
        else:
            sparsity_scale = 1.
