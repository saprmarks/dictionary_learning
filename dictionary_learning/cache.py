...
class ActivationShard:
    def __init__(self, store_dir : str, shard_idx : int):
        ...
        self.activations = np.memmap(self.shard_file, dtype=np.float32, mode='r', shape=self.shape)
...
class ActivationCache:
    def __init__(self, store_dir : str):
        ...
        self.shards = [ActivationShard(store_dir, i) for i in range(self.config["shard_count"])]
    ...
    def collate_store_shards(store_dirs : Tuple[str], shard_count : int, activation_cache : List[th.Tensor], submodule_names : Tuple[str], shuffle_shards : bool = True, io : str = "out"):
        for i, name in enumerate(submodule_names):
                ...
                memmap = np.memmap(memmap_file, dtype=np.float32, mode='w+', shape=(activations.shape[0], activations.shape[1]))
    ...
    def collect(data : Dataset, 
        ...
        store_dirs = [os.path.join(store_dir, f"{submodule_names[i]}_{io}") for i in range(len(submodules))]
        ...
        for batch in tqdm(dataloader, desc="Collecting activations"):
            tokens = model.tokenizer(batch, max_length=context_len, truncation=True, return_tensors="pt", padding=True).to(model.device)
            ...
            with model.trace(
                ...
                **tracer_kwargs,
                ...
                for i, submodule in enumerate(submodules):
                    local_activations = ActivationCache.get_activations(submodule, io).reshape(-1, d_model).save() # (B x T) x D
            ...
            for i in range(len(submodules)):
                activation_cache[i][-1] = activation_cache[i][-1].value[attention_mask.reshape(-1).bool()].cpu().to(th.float32) # remove padding tokens
